...
// Cookitter Origin is a tag attached to all cookitter generated elements.
// It's used to attach a semantic inforation to normal Adobe elements.
// In our objects three the same elemement can have different meaning.
// First of all, all elements generated by cookitter have an origin tag with
// a value that starts with  `cookie`, this allows to distingush them from the
// user created one that must not have the tag.
// PathItems and PlacedItems generated have a different tag, while it's not needed now
// it could be needed to distinguish between differetn usages of the same object type.
// Groups have different purpose:
// - Main groups collect the clipping groups generated from the same user PathItem
// - Clip groups pairs each PathItem with clipped PlacedItem
const cookitterTagNameOrigin = "cookitter_origin";
// Cookitter Tag Name Origin Values
const ctno = {
  pathItemCookie: "cookie-path-item-cookie",
  placedItemCookie: "cookie-placed-item-cookie",
  gmain: "cookie-group-main",
  gclip: "cookie-group-clip",
};

function startsWithCookie(str: string | null): boolean {
  return startsWith("cookie-", str);
}
...
function resetPageItems(obj: Layer | GroupItem, parent: Layer | GroupItem) {
  ...
  for (let i = obj.pageItems.length - 1; i >= 0; i--) {
    ...
    if (objType == "GroupItem") {
      // we just checked that the element is a GroupItem
      // @ts-ignore
      ...
      if (!origin) {
        ...
        // We should cycle through the remaining items and move them
        // to the latest valid parent
        for (let j = groupItem.pageItems.length - 1; j >= 0; j--) {
          groupItem.pageItems[j].move(
            parent,
            // ElementPlacement.PLACEATEND is the right value, types are wrong
            // @ts-ignore
...
function makeCookitterMainGroup(obj: PathItem): GroupItem {
  l.i("makeCookitterMainGroup...");
  // ts types hole...
  // @ts-ignore
  const parent: Layer | GroupItem = obj.parent;
  const mainGroup: GroupItem = parent.groupItems.add();
  ...
  // ElementPlacement.PLACEATBEGINNING is the right value, types are wrong
  // @ts-ignore
    ...
    if (origin == ctno.gclip) {
      ...
      if (group.pathItems.length == 1) {
        ...
        if (!piOrigin) {
          ...
          eg.source = pi;
          type = "source";
        } else if (piOrigin == ctno.pathItemCookie) {
...
function syncItems(artBag: ArtboardsBag, eg: ElementGroup) {
  ...
  if (sArtboardIdx !== null) {
    ...
    // It must be a PathItem because it's cloned from a PathItem
    // @ts-ignore
    const newDPi: PathItem = sPi.duplicate(
      eg.destinationGroup,
    ...
    setTagValue(newDPi, cookitterTagNameOrigin, ctno.pathItemCookie);
